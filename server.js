...
app.get('/api/stats', async (req, res) => {
    try {
        ...
        res.json({ 
            ...
            temperature: (cpuTemp.main > 0) ? Math.round(cpuTemp.main) : 45, 
            config: readConfig() 
        });
...
app.get('/api/services', async (req, res) => {
        ...
        fs.readFile(portFile, 'utf8', (err, data) => {
            if (err) {
                exec("ss -tuln | grep -E '8888|81|82'", (err2, stdout) => {
                    systemServices.push({ id: "SYS-01", name: "aaPanel", image: "Native OS", state: stdout ? "running" : "stopped", ports: stdout ? "Detected" : "N/A", type: "system" });
                    resolve();
                });
            } else {
                const port = data.trim();
                exec(`ss -tuln | grep :${port}`, (err3, stdout) => {
                    systemServices.push({ id: "SYS-01", name: "aaPanel", image: "Native OS", state: stdout ? "running" : "stopped", ports: port, type: "system" });
                    resolve();
                });
            }
        });
    ...
    containers.forEach(c => {
        ...
        dockerServices.push({ id: c.Id.substring(0, 12), name: c.Names[0].replace(/^\//, ''), image: c.Image, state: c.State, ports: [...new Set(portMap)].join(', ') || 'No Port', type: "docker" });
    });
    res.json({ system: systemServices, docker: dockerServices });
});

app.listen(PORT, '0.0.0.0', () => console.log(`Pusat Kendali berjalan di port: ${PORT}`));